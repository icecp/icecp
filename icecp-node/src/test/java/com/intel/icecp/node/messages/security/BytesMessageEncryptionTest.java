 ...
 */
@Ignore
public class BytesMessageEncryptionTest {
    ...
    public void test() throws Exception {
        ...
        Format<BytesMessage> ecryptedMsgFormat = FormatUtils.getMessageFormatter(f, BytesMessage.class);
        ...
        logger.debug("Encrypted message: {}", new String(StreamUtils.readAll(ecryptedMsgFormat.encode(encryptedMessage))));

        // TEST: DECRYPT
        ecryptedMsgFormat = FormatUtils.getMessageFormatter(f, BytesMessage.class);

        InputStream is = ecryptedMsgFormat.encode(encryptedMessage);

        // First retrieve the encrypted message
        BytesMessage encMsg = ecryptedMsgFormat.decode(is);
        ...
        logger.debug("Decrypted message: {}", new String(StreamUtils.readAll(new ByteArrayInputStream(bytes))));
